...
def phoneme_extraction_test(audio_input, word):
    df = pd.DataFrame(algorithms.measureFormants(
        audio_input))  # Frequency range[0] -> 0 means the first word. In the audio, there are three words.
    ...
    for i in range(len(dataFrame)):
        for x in range(len(vowels)):
            if vowels[x]['f1_low'] < dataFrame['f1'][i + 1] < vowels[x]['f1_high'] and \
                    vowels[x]['f2_low'] < dataFrame['f2'][i + 1] < vowels[x]['f2_high']:
