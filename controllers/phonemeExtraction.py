...
def phoneme_extraction(audio_input, word):
    ...
    df = pd.DataFrame(frequency_range[0])#Frequency range[0] -> 0 means the first word. In the audio, there are three words.
    ...
    for i in range(len(dataFrame)):
        for x in range(len(vowels)):
            if dataFrame['f1'][i] > vowels[x]['f1_low'] and dataFrame['f1'][i] < vowels[x]['f1_high'] and dataFrame['f2'][i] > vowels[x]['f2_low'] and dataFrame['f2'][i] < vowels[x]['f2_high']:
