...
def phoneme_extraction(audio_input, word):
    frequency_range = controllers.get_frequency_range(audio_input)

    df = pd.DataFrame(
        frequency_range[0])  # Frequency range[0] -> 0 means the first word. In the audio, there are three words.
    ...
    for i in range(len(dataFrame)):
        for x in range(len(vowels)):
            if vowels[x]['f1_low'] < dataFrame['f1'][i] < vowels[x]['f1_high'] and vowels[x]['f2_low'] < \
                    dataFrame['f2'][i] < vowels[x]['f2_high']:
                pronounced_vowel.append(vowels[x]['phone'])

    vowel_count = {}

    for ws in pronounced_vowel:
        if ws not in vowel_count:
            vowel_count[ws] = 1
        else:
            vowel_count[ws] += 1

    # print(len(pronounced_vowel))

    # for x in vowel_count:
    #     percentage = (vowel_count[x] / len(pronounced_vowel)) * 100
